...
public class SkillManager : SingletonMonoBehaviour<SkillManager>
{
    ...
    public void SetSkillBase(Equipment[] equipments)
    {
        ...
        Debug.Log("Equipment 1 : " + equipments[0].equipmentName);
        Debug.Log("Equipment 2 : " + equipments[1].equipmentName);
        skills[0] = equipments[0].skill;
        skills[1] = equipments[1].skill;
    }

    //ゲーム開始時のスキルの実行
    public void TriggerOnGameStart(MagicCreator[] magics, WizardModel model)
    {
        foreach (var skill in skills.OfType<IOnGameStart>())
            skill.OnGameStart(magics, model);
    }
    ...
    public void TriggerOnDamage(WizardModel model)
    {
        foreach (var skill in skills.OfType<IOnDamage>())
            skill.OnDamage(model);
    }

    public void TriggerOnHeal(WizardModel model)
    {
